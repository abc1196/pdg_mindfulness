...
public abstract class ImageProcessing {
    private static int decodeYUV420SPtoRedSum(byte[] yuv420sp, int width, int height, double threshold) {
     ...
     */
    public static int decodeYUV420SPtoRedAvg(byte[] yuv420sp, int width, int height, double threshold) {
        if (yuv420sp == null) return 0;

        final int frameSize = width * height;

        int sum = decodeYUV420SPtoRedSum(yuv420sp, width, height, threshold);
